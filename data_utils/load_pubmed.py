...
def get_pubmed_casestudy(corrected=False, SEED=0):
    _, data_X, data_Y, data_pubid, data_edges, data_TL = parse_pubmed()
...
def parse_pubmed():
    path = '/gpfsnyu/scratch/ys6310/benchmark/dataset/PubMed_orig/data/'
    ...
    data_TL = [None] * n_nodes
    data_edges = []

    paper_to_index = {}
    feature_to_index = {}
    true_labels = ["Experimentally induced diabetes", "Type 1 diabetes", "Type 2 diabetes"]
    # parse nodes
    ...
    return data_A, data_X, data_Y, data_pubid, np.unique(data_edges, axis=0).transpose(), data_TL
...
def get_raw_text_pubmed(use_text=False, seed=0):
    ...
    f = open('/gpfsnyu/scratch/ys6310/benchmark/dataset/PubMed_orig/pubmed.json')
    pubmed = json.load(f)
