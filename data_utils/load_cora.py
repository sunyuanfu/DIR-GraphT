...
def get_cora_casestudy(SEED=0):
    data_X, data_Y, data_citeid, data_edges, data_TL = parse_cora()
...
def parse_cora():
    path = '/gpfsnyu/scratch/ys6310/benchmark/dataset/cora_orig/cora'
    ...
    return data_X, data_Y, data_citeid, np.unique(data_edges, axis=0).transpose(), data_TL
...
def get_raw_text_cora(use_text=False, seed=0):
    ...
    with open('/gpfsnyu/scratch/ys6310/benchmark/dataset/cora_orig/mccallum/cora/papers')as f:
        lines = f.readlines()
    pid_filename = {}
    for line in lines:
        pid = line.split('\t')[0]
    ...
    path = '/gpfsnyu/scratch/ys6310/benchmark/dataset/cora_orig/mccallum/cora/extractions/'
    text = []
